    ...
    value: function render() {
      ...
      var links = [{
        ...
        action: function action() {
          _this.app.replace(_this.context.app.route(bootstrap.getPrefix(_this.context.app)).resolvePath);
        }
      }];
      var data = exceptions[this.props.route.exception || 404];
      data.desc = locale[data.title];
      return React.createElement(Exception, {
        data: data,
        style: {
          minHeight: 500,
          height: '80%'
...
ExceptionScene = __decorate([localeContext('Exception', {
  backToHome: '返回首页',
  403: '抱歉，你无权访问该页面',
  404: '抱歉，你访问的页面不存在',
  500: '抱歉，服务器出错了'
})], ExceptionScene);
export var fallbackRoutes = [{
  name: 'noAuth',
