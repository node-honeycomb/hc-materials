...
import { getComponent as _getComponent } from '../layouts/getComponent';
  ...
  _createClass(Layer, [{
    ...
    value: function getComponent(component, props, getProps) {
      ...
      if (component.ChildComponent) {
        ...
        component.ChildComponent = _getComponent(props, getProps)(component.ChildComponent);
        component.ChildComponent.contextTypes = contextTypes;
        newComponent = component;
      } else {
        contextTypes = component.contextTypes;
        newComponent = _getComponent(props, getProps)(component);
    ...
    value: function parseComponent(opt, state) {
      ...
      var newComponent;
      var props = opt.props;
      var getProps = converter.parse(opt.getProps);
      var cname = opt.cname;
      var componentOption = {
        "static": opt["static"],
        attrs: opt.attrs,
      ...
      if (opt.layer) {
        ...
        if (Array.isArray(cname)) {
          newComponent = getCombox(opt, findComponent, function () {
    ...
    value: function toView(component, viewOpt, props, getProps) {
      ...
      if (viewOpt) {
        ...
        if (viewOpt.providers) {
          ...
          forEach(viewOpt.providers, function (provider, name) {
    ...
    value: function getLayer(opt, getState) {
      ...
      return Promise.all([promise, this._promise]).then(function (_ref) {
        ...
        if (isEqualWith(nextState, opt._prevState, _this.customizer) && opt._children) {
    ...
    value: function parseLayer(components) {
      ...
      var layerProps = cloneDeep(props); // TODO LayerItem提供了拖拽功能
      ...
      if (components) {
        var index = 0;
        forEach(components, function (com, key) {
    ...
    value: function parseRoute(layout) {
      ...
      if (routeOptions.layoutOption.components) {
        var newComponents = {};
        forEach(routeOptions.layoutOption.components, function (com, key) {
    ...
    value: function createLayerRoutes(scenes) {
      ...
      var layerRoutes = [];
      forEach(scenes, function (sceneOption) {
