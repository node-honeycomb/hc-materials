  ...
  if (Array.isArray(arrOrMap)) {
    ...
    for (var key in arrOrMap) {
      ...
      if (typeof item === 'function') {
        if ((0, _isReactComponent["default"])(item)) {
...
BasicLayout.getLayoutProps = function (option, viewContent) {
  ...
  loop(option.components, function (item) {
    ...
    if (Com) {
      ...
      layoutProps.components[item.key] = layoutProps.components[item.key] || (0, _getComponent2.getComponent)(item.props, item.getProps, item.contextTypes)(Com);
